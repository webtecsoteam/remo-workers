...
try {
    ...
    $cStmt = $db->prepare("SELECT * FROM contracts WHERE id = ? AND freelancer_id = ?");
    ...
    if ($contract['status'] === 'completed' || $contract['status'] === 'paused') {
        // Automatically reactivate contract when proposing a new milestone
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Milestone proposed and added successfully!',
